...
variable "remote_state_bucket" {
  description = "S3 bucket holding the cluster module's terraform state. This module reads <account_name>/<region>/rds/cluster/<cluster_identifier>/terraform.tfstate for the cluster's outputs (DESIGN-0014 / ADR-0001 — remote-state composition; account-scoped key per IMPL-0015)."
  type        = string
  nullable    = false
}
...
variable "account_name" {
  description = "Terragrunt account name — the <account_name> prefix of the account-scoped remote-state key this module reads (<account_name>/<region>/rds/cluster/<cluster_identifier>/terraform.tfstate)."
  type        = string
  nullable    = false
}
