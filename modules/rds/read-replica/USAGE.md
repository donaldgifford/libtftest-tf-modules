...
| account\_name | Terragrunt account name — the <account\_name> prefix of the account-scoped remote-state key this module reads (<account\_name>/<region>/rds/cluster/<cluster\_identifier>/terraform.tfstate). | `string` | n/a | yes |
...
| remote\_state\_bucket | S3 bucket holding the cluster module's terraform state. This module reads <account\_name>/<region>/rds/cluster/<cluster\_identifier>/terraform.tfstate for the cluster's outputs (DESIGN-0014 / ADR-0001 — remote-state composition; account-scoped key per IMPL-0015). | `string` | n/a | yes |
