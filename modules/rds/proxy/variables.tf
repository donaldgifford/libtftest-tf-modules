...
variable "remote_state_bucket" {
  description = "S3 bucket holding the target DB module's terraform state. The proxy reads <account_name>/<region>/rds/<dir>/<target_identifier>/terraform.tfstate for the target's outputs (DESIGN-0010 Q3 — remote-state composition; account-scoped key per IMPL-0015)."
  type        = string
  nullable    = false
}
...
variable "account_name" {
  description = "Terragrunt account name — the <account_name> prefix of the account-scoped remote-state key this module reads (<account_name>/<region>/rds/<dir>/<target_identifier>/terraform.tfstate)."
  type        = string
  nullable    = false
}
