...
#
# The instance module reads vpc_id + private_subnet_ids from a
# remote-state file in S3 (data.terraform_remote_state.vpc per
# IMPL-0007 Q1). This fixture builds:
#   1. A VPC + 3 private subnets in three AZs (the db subnet group needs
#      subnets in at least 2 AZs).
#   2. An S3 bucket holding the stub VPC state file at the key
#      <region>/vpc/<vpc_name>/terraform.tfstate, with outputs shaped to
#      match the EKS-cluster remote-state contract.
#
...
resource "aws_subnet" "private" {
  ...
  tags = {
    Name = "tftest-rds-instance-private-${count.index}"
    Tier = "private"
  }
}
...
resource "aws_s3_object" "vpc_state" {
  ...
  content = jsonencode({
    ...
    serial            = 1
    lineage           = "tftest-rds-instance-stub-vpc"
    outputs = {
      vpc_id = {
