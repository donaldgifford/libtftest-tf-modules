...
# per DESIGN-0007 Q5 / IMPL-0007 Phase 4: callers pass consumer SG
# IDs via var.allowed_consumer_sg_ids; the module emits one granular
# aws_vpc_security_group_ingress_rule per entry on the resolved port
# (var.db_port override or the engine default). Empty list leaves the
# instance reachable from nowhere.
#--------------------------------------------------------------

resource "aws_db_subnet_group" "this" {
  name        = "${var.identifier_prefix}-rds-instance"
  description = "RDS instance ${var.identifier_prefix} subnet group"
...
resource "aws_security_group" "this" {
  name        = "${var.identifier_prefix}-rds-instance"
  description = "RDS instance ${var.identifier_prefix} security group"
...
resource "aws_vpc_security_group_ingress_rule" "consumer" {
  ...
  ip_protocol                  = "tcp"
  description                  = "Ingress from consumer SG ${each.value} on the resolved DB port"
  tags                         = var.tags
}
