...
variable "engine" {
  description = "Aurora engine: 'aurora-postgresql' or 'aurora-mysql'. The module rejects non-Aurora engines (those belong to the future modules/rds/instance module)."
  type        = string
...
variable "min_acu" {
  description = "Minimum Aurora Capacity Units (ACUs) for the Serverless v2 scaling configuration. Range: 0.5 - 256. Suggested starting points: dev = 0.5; production starter = 0.5. Tune to your workload's load shape (load below this floor still costs min_acu * $0.12/hour for Postgres in us-east-1)."
  type        = number

  validation {
    condition     = var.min_acu >= 0.5 && var.min_acu <= 256
    error_message = "min_acu must be in the range [0.5, 256] (Aurora Serverless v2 hard bounds)."
  }
...
variable "max_acu" {
  description = "Maximum Aurora Capacity Units (ACUs) for the Serverless v2 scaling configuration. Range: 0.5 - 256. Suggested starting points: dev = 4; production starter = 16. Tune to your workload's peak load shape. min_acu <= max_acu is enforced via a precondition on the cluster resource."
  type        = number

  validation {
    condition     = var.max_acu >= 0.5 && var.max_acu <= 256
    error_message = "max_acu must be in the range [0.5, 256] (Aurora Serverless v2 hard bounds)."
  }
...
variable "performance_insights_enabled" {
  description = "Opt-in Performance Insights on the cluster instance. Default false (per IMPL-0007 Q6 — conservative on cost; caller opts in). When true, PI uses local.kms_key_arn for encryption."
  type        = bool
  default     = false
}
