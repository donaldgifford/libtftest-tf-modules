...
#   2. proxy (target_type = "aurora-cluster") reads the proxy
#      composition set.
#
...
output "reader_endpoint" {
  description = "Reader endpoint hostname for the cluster. Aurora distributes read traffic across cluster instances; with a single writer instance, this resolves to the same endpoint as cluster_endpoint until the read-replica module adds readers."
...
output "db_subnet_group_name" {
  description = "Name of the DB subnet group created for this cluster. Read by sibling RDS modules that share the same subnet topology (the read-replica module consumes this through remote state)."
...
output "db_cluster_parameter_group_name" {
  description = "Name of the cluster parameter group created for this cluster. The read-replica module consumes this through remote state so replicas share the cluster's parameter family."
  value       = aws_rds_cluster_parameter_group.this.name
}

output "db_parameter_group_name" {
  description = "Name of the instance parameter group attached to the provisioned writer instance."
  value       = aws_db_parameter_group.this.name
}

output "cluster_instance_identifier" {
  description = "Identifier of the provisioned writer cluster instance (<identifier_prefix>-1). Useful for AWS CLI / SDK operations targeting the instance directly (e.g., reboot-db-instance)."
...
# state to place an RDS Proxy in front of it. The serverless / instance
# siblings emit the same four outputs so a single proxy module can front
# any target_type.
#--------------------------------------------------------------
