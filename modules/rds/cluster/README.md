...
```text
<account_name>/<region>/rds/cluster/<identifier_prefix>/terraform.tfstate
```

`identifier_prefix` is load-bearing three ways: it is the cluster
identifier, the live-repo folder name, and the identifier both `rds/proxy`
(`target_identifier`) and `rds/read-replica` (`cluster_identifier`) use to
find this cluster's state. A mismatch fails the consumer plan with
`Unable to find remote state` (the error names neither bucket nor key —
diff against this contract). The key templates are pinned by plan
