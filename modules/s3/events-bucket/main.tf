# S3 events bucket (modules/s3/events-bucket)
#
# The family's event-producing bucket (DESIGN-0019 Phase 4 / INV-0009
# F1): everything `s3/bucket` is — the F2 security baseline from the
# internal core, the F4 access-logging tri-state, the six Terragrunt
# globals, additive operator policy statements (OQ 4b) — plus the
# notification surface in notification.tf (typed SQS/SNS destination
# lists + EventBridge, OQ 5a).
#
# Destination resource policies (the SQS queue policy / SNS topic
# policy allowing s3.amazonaws.com from this bucket's ARN) are owned
# by the DESTINATION stacks, not this module — see the README and the
# apply fixture for the required shape.
#
