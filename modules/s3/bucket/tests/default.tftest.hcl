# Tri-state + contract pins (IMPL-0018 3.3): all three access-logging
# paths, the ADR-0020 reserved-key assertion, and the additive policy
# merge. account_name/account_id/region come from the shared var-file
...
run "default_lookup" {
  ...
  assert {
    condition     = output.bucket_name == "app-data-000000000000-us-east-1"
...
run "additional_statements_additive" {
  ...
  variables {
    ...
    additional_policy_statements = [{
      sid        = "AllowCiReadOnly"
      principals = { AWS = ["arn:aws:iam::000000000000:role/ci-reader"] }
      actions    = ["s3:GetObject"]
    }]
  }

  assert {
    condition     = contains([for s in jsondecode(output.bucket_policy_json).Statement : s.Sid], "AllowCiReadOnly")
  ...
  # Default resource_suffixes ["", "/*"] expand against the composed
  # bucket ARN (an input cannot reference the module's own output).
  assert {
    condition = toset(one([for s in jsondecode(output.bucket_policy_json).Statement : s if s.Sid == "AllowCiReadOnly"]).Resource) == toset([
      "arn:aws:s3:::app-data-000000000000-us-east-1",
      "arn:aws:s3:::app-data-000000000000-us-east-1/*",
    ])
